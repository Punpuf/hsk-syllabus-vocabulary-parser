...
The goal of this project is to produce a list of Chinese words per **HSK level** where **both Hanzi and pinyin are present**. Many existing lists only store Hanzi, which is not sufficient for uniquely identifying a word because multiple words can share the same Hanzi with different pronunciations.
...
You can simply [download the generated TSV file](./hsk_word_list.tsv), but if you want to run the script yourself, follow these steps:
...
- A single sense (`...1` or `...2`) can appear in up to two HSK levels.
- The part-of-speech can also change across those levels.

Examples from the generated TSV (the _meanings_ are AI generated and may very well be incorrect, this was included only to help illustrate):
...
- **Other PDF extraction issues**: a small override table patches known missing Hanzi cases where the PDF layout extraction fails.
